	...
	const writeIndexJsFiles = (src: string) => {
		...
		for (const typeChainExport of typeChainExports) {
			...
			if (typeChainExport.defaultExportName !== undefined) {
				...
				rootExports.push(
					`export { default as ${name} } from './${typeChainExport.dir}/${file}'`,
				);
			}
		...
		const argumentTypes: ExportType[] = Array.from(
		...
		const returnTypes: ExportType[] = Array.from(
		...
		uniqueTypes.forEach((entry, i) => {
			...
			const prefix =
				entry.category === "type" || entry.category === "interface"
					? " type"
					: "";
			rootExports.push(
