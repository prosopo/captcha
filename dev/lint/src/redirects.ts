...
const findWorkspaceRoot = (): string => {
  ...
  console.warn('Warning: Could not find workspace root. Using current directory.');
  return currentDir;
};
...
const needsTrailingSlash = (url: string): boolean => {
  ...
  const fileExtensions = ['.html', '.htm', '.xml', '.json', '.js', '.css', '.jpg', '.jpeg', '.png', '.gif', '.pdf', '.svg', '.txt'];
 ...
 */
const extractMarkdownLinks = (content: string): { url: string, line: number, originalText: string }[] => {
 ...
 * @returns Array of URLs and their line numbers
 */
const extractHtmlLinks = (content: string): { url: string, line: number, originalText: string }[] => {
  const links: { url: string, line: number, originalText: string }[] = [];
  const lines = content.split('\n');
    ...
    const allLinks = [...mdLinks, ...htmlLinks].map(link => {
      ...
      if (needsSlash) {
        ...
        if (link.originalText.includes(`"${link.url}"`)) {
          replacementText = link.originalText.replace(`"${link.url}"`, `"${urlWithSlash}"`);
 ...
 */
export const findTemplateFiles = async (targetDir: string): Promise<string[]> => {
...
const fixFile = (filePath: string, linksToFix: FileLink[]): void => {
  ...
  try {
    ...
    const lineGroups = linksToFix.reduce<{[key: number]: FileLink[]}>((groups, link) => {
    ...
    for (const lineNumber of lineNumbers) {
      ...
      for (const link of linksInLine) {
        if (link.originalText && link.replacementText && link.originalText !== link.replacementText) {
          lineContent = lineContent.replace(link.originalText, link.replacementText);
        }
      }
  ...
  try {
    ...
    if (linksWithoutSlash.length > 0) {
      if (mode === Mode.FIX) {
        ...
        linksWithoutSlash.forEach(link => {
        ...
        linksWithoutSlash.forEach(link => {
          ...
          console.error(`  - ${relativePath}:${link.line} - ${link.url} (should be ${link.url}/)`);
        });
