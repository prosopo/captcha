...
function clickIAmHuman(): Cypress.Chainable<Captcha[]> {
    ...
    cy.get(webComponentTag).shadow().find(checkboxClass, {timeout: 12000}).first().click();
...
function clickCorrectCaptchaImages(
    ...
    return cy.captchaImages().then(() => {
        cy.getSelectors(captcha).then((selectors: string[]) => {
            ...
            cy.get(webComponentTag).shadow().find(selectors.join(", ")).then((elements) => {
...
function clickNextButton() {
    ...
    cy.get(webComponentTag).shadow().find('button[data-cy="button-next"]').click({force: true});
    cy.wait(0);
}
