...
describe('Captchas', () => {
    ...
    it.only("Captchas load when 'I am human' is pressed", async () => {
        const captchas = await cy.clickIAmHuman().promisify()
        cy.log('wtfff')
        expect(captchas.length).to.be.gt(0)
    })

    it.only('Number of displayed captchas equals number received in response', async () => {
        const captchas = await cy.clickIAmHuman().promisify()
        cy.wait(2000)
        const selectAll = await cy.xpath('"//p[contains(text(),\'images containing\')]"')
        const container = await selectAll.parent().parent().children()
        cy.log('here!')
        cy.log(container)

        // const imageDivs = container[1].children()[0].children()
        // imageDivs.should('have.length', captchas.length)
    })
    ...
    it('Can select an item', () => {
        ...
        cy.get('[data-cy="captcha-0"]').find("[data-cy='captcha-item']").first().click()
