...
import type { SearchParams, Hotel, SearchResponse, ApiError } from '../frontend/types';
...
const PROSOPO_SERVER_URL = process.env.PROSOPO_SERVER_URL || 'http://localhost:9229';
const PROSOPO_SITE_KEY = process.env.PROSOPO_SITE_KEY || '5GCP69mhanZqJqnTvaaGvJCJZWSahz6xE2c7bTGETqSB5KDK';
...
const MOCK_HOTELS: Hotel[] = [
  {
    ...
    description: 'Luxury 5-star hotel in the heart of the city with stunning views, world-class amenities, and exceptional service.',
    image: 'https://images.unsplash.com/photo-1566073771259-6a8506099945?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=1000&q=80',
    rating: 5,
    reviewCount: 342,
    price: 299,
    currency: '$',
    amenities: ['Free WiFi', 'Pool', 'Spa', 'Fitness Center', 'Restaurant', 'Room Service'],
    location: {
  ...
  },
  {
    ...
    description: 'Modern boutique hotel with stylish rooms, rooftop bar, and personalized service in a prime downtown location.',
  ...
  },
  {
    ...
    description: 'Comfortable and affordable hotel perfect for business travelers and tourists exploring the city.',
  ...
  },
  {
    ...
    description: 'Tranquil resort by the river featuring spacious suites, multiple dining options, and recreational facilities.',
    ...
    price: 249,
    currency: '$',
    amenities: ['Pool', 'Spa', 'Golf Course', 'Multiple Restaurants', 'Conference Rooms'],
    location: {
...
async function validateProcaptchaToken(token: string): Promise<boolean> {
  try {
    const response = await fetch(`${PROSOPO_SERVER_URL}/v1/siteverify`, {
      method: 'POST',
      headers: {
...
app.post('/api/search', async (req, res) => {
  try {
    const { procaptchaResponse, ...searchParams }: SearchParams & { procaptchaResponse: string } = req.body;
...
app.get('/api/demo-info', (req, res) => {
  res.json({
    ...
    description: 'This demo showcases how Procaptcha protects APIs from automated scraping and abuse.',
    features: [
...
// Start server
app.listen(PORT, () => {
  console.log(`🔒 API Protection Demo Server running on http://localhost:${PORT}`);
