...
export const Manager = (
    ...
    const getConfig = () => {
        ...
        if (state.account) {
            debug('Using account from state', { address: state.account.account.address });
    ...
    const resetState = (frictionlessRestart?: () => void) => {
        ...
        debug('Clearing mouse movement data', { previousCount: state.mouseMovements.length });
    ...
    const setValidChallengeTimeout = () => {
        ...
        debug('Valid challenge timeout set', { timeout: successfullChallengeTimeout });
    };
    ...
    const setChallengeTimeout = () => {
        debug('Setting challenge solution timeout', { timeoutMs: SOLUTION_TIMEOUT });
    ...
    const trackMouseMovement = (x: number, y: number) => {
        ...
        if (state.mouseMovements.length % 10 === 0) {
            debug('Tracking mouse movement', { 
                ...
                timeElapsed: state.captchaStartTime ? Date.now() - state.captchaStartTime : 'unknown'
            });
        }
    };
     ...
     */
    const verifySolution = async (sliderPosition: number, targetPosition: number) => {
        ...
        if (state.mouseMovements.length > 5) {
            ...
            const yAvg = yPositions.reduce((a: number, b: number) => a + b, 0) / yPositions.length;
            yVariance = yPositions.reduce((a: number, b: number) => a + Math.pow(b - yAvg, 2), 0) / yPositions.length;
        ...
        try {
            ...
            debug('Fingerprint generated', { fingerprintPrefix: fingerprint.substring(0, 8) + '...' });
        } catch (error) {
    ...
    const start = async () => {
        ...
        try {
            ...
            debug('Incrementing attempt counter', { previousCount: state.attemptCount, newCount: newAttemptCount });
            updateState({ attemptCount: newAttemptCount });
            
            const config = getConfig();
            debug('Got configuration', { 
                web2: config.web2, 
                userAccountAddress: config.userAccountAddress?.substring(0, 8) || 'none'
            });
            ...
            const selectAccount = async () => {
                ...
                // For web2, create an anonymous account based on fingerprint
                if (config.web2) {
                    debug('Creating anonymous account using fingerprint (web2 mode)');
                    const fingerprint = await getFingerprint();
                    debug('Fingerprint generated', { fingerprintPrefix: fingerprint.substring(0, 8) + '...' });
                    return {
                        account: { address: `anon-${fingerprint.substring(0, 16)}` }
                    };
                }
                
                // For web3, use extension account
                debug('Loading extension for account selection (web3 mode)');
            ...
            debug('Setting dapp account in state', { dappAccount: config.account.address });
            ...
            try {
                ...
                const sliderCaptchaResponse = await providerApi.getSliderCaptchaChallenge(
                    userAccount,
            ...
            debug('Started tracking solve time', { startTime: state.captchaStartTime });
            ...
            updateState({
                loading: false,
                error: {
                    message: error instanceof Error ? error.message : "Unknown error starting captcha",
    ...
    const onSuccess = async (sliderPosition: number, targetPosition: number) => {
        ...
        try {
            ...
            debug('Verification token generated', { tokenLength: verificationToken.length });
            ...
            if (isVerified) {
                ...
                const fingerprint = await getFingerprint();
                debug('Fingerprint for token generated', { fingerprintPrefix: fingerprint.substring(0, 8) + '...' });
                ...
                debug('Client-side verification token generated', { tokenLength: verificationToken.length });
