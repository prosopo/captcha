...
export const SliderCaptchaWidget = (props: ProcaptchaProps) => {
	...
	updateBaseStateRef.current = updateBaseState;
	...
	const theme =
		"light" === stateRef.current.config.theme ? lightTheme : darkTheme;
	...
	useEffect(() => {
		...
		const handleExecuteEvent = (event: Event) => {
			...
			if (config.mode === "invisible") {
				console.log(
					"[SliderCaptcha] Invisible mode - requesting new challenge",
				);
	...
	const resetPuzzle = useCallback(async () => {
		...
		try {
			...
			if (typeof challenge.targetPosition === "number") {
				...
				challenge.targetPosition2D &&
				typeof challenge.targetPosition2D.x === "number"
			) {
				// Object format with x/y coordinates
				puzzleDestination = challenge.targetPosition2D.x;
			} else if (
				challenge.targetPosition &&
				typeof challenge.targetPosition === "object" &&
				"x" in challenge.targetPosition
			) {
				// GetSliderCaptchaResponse format
				puzzleDestination = (
					challenge.targetPosition as { x: number; y: number }
				).x;
			} else {
	...
	const generateRandomImage = (destination: number, retryCount = 0) => {
		...
		img.onerror = () => {
			...
			console.error(
				`Failed to load image (attempt ${retryCount + 1}/${MAX_IMAGE_RETRIES}), trying another one`,
			);
	...
	const handleVerificationFailure = () => {
		...
		if (
			...
			callbacks &&
			"onFailed" in callbacks &&
			typeof callbacks.onFailed === "function"
		) {
			callbacks.onFailed();
		}
			...
			if (config.mode !== "invisible") {
				...
				setTimeout(() => {
					...
					const checkbox = document.querySelector(
						'.slider-captcha input[type="checkbox"]',
					) as HTMLInputElement;
					if (checkbox) {
						checkbox.click();
					}
	...
	const handleDragEnd = (e: MouseEvent | TouchEvent) => {
		...
		manager
			.onSuccess(finalPosition, targetPosition)
			.then((isVerified) => {
	...
	const handleChallengeUpdate = useCallback(() => {
		...
		if (stateRef.current.challenge) {
			...
			if (
				...
				console.log(
					"[SliderCaptcha] No image provided in challenge, generating random one",
				);
			...
			console.log(
				"[SliderCaptcha] No challenge provided, generating random one",
			);
	...
	const renderShapedSliderCaptcha = (
		...
		const completeRendering = () => {
			...
			if (stateRef.current?.challenge?.targetPosition2D?.y !== undefined) {
				...
				stateRef.current?.challenge?.targetPosition &&
				typeof stateRef.current.challenge.targetPosition === "object" &&
				"y" in stateRef.current.challenge.targetPosition
			) {
				targetY = (
					stateRef.current.challenge.targetPosition as { x: number; y: number }
				).y;
			}
			...
			if (block) {
				...
				console.log(
					`[SliderCaptcha] Setting puzzle piece Y position to ${targetY}px`,
				);
			}
		...
		baseImage.onerror = () => {
			console.error(
				"[SliderCaptcha] Failed to load base image, falling back to random image",
			);
		...
		puzzlePieceImage.onerror = () => {
			console.error(
				"[SliderCaptcha] Failed to load puzzle piece image, falling back to random image",
			);
			generateRandomImage(targetPosition);
		};
