...
const render = async (widgetProps: ProcaptchaProps): Promise<void> => {
	await act(async () => {
		root.render(
			createElement(entrypoint.ProcaptchaAudio, widgetProps) as ReactElement,
		);
	});
};
...
describe("the package entrypoint", () => {
	test("exports the wrapper a consumer mounts", () => {
		...
		expect(typeof entrypoint.ProcaptchaAudio).toBe("function");
		expect(Object.keys(entrypoint)).toEqual(["ProcaptchaAudio"]);
	});
});
