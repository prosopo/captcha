...
interface AudioManagerHandle {
	start: (
		x?: number,
		y?: number,
	) => Promise<GetAudioCaptchaResponse | undefined>;
	submitSolution: (
		answer: string,
		replays: number,
...
export const Manager = (
	...
	let storedChallengeResponse: GetAudioCaptchaResponse | undefined;
		...
		await providerRetry(
			async () => {
				...
				const simdReadingsOnChallenge = frictionlessState?.getSimdReadings
					...
					: undefined;
				const challenge = await providerApi.getAudioCaptchaChallenge(
					userAccount,
					getDappAccount(),
				...
				// Set loading to false to signal the widget to show the player
				updateState({
					loading: false,
				});
			},
	...
	const submitSolution = async (
		...
		try {
			...
			// records the (x, y) on the audio captcha record for telemetry.
			...
			const verifiedSolution = await providerApi.submitAudioCaptchaSolution(
				challenge,
