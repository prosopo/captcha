...
export async function batch(
    ...
    const nonce = (
        await contract.api.rpc.system.accountNextIndex(pair.address)
    ).toNumber()
    ...
    const runtimeVersion =
        await contract.api.rpc.state.getRuntimeVersion(blockHash)
    ...
    return await new Promise(async (resolve, reject) => {
        const unsub = await batchExtrinsic.signAndSend(
            ...
            async (result: SubmittableResult) => {
                ...
                const batchInterruptedEvent = result.events.filter(
                    (e) => e.event.method === 'BatchInterrupted'
                )
                const tooManyCallsEvent = result.events.filter(
                    (e) => e.event.method === 'TooManyCalls'
                )
                const extrinsicSuccess = result.events.filter(
                ...
                if (result.status.isFinalized || result.status.isInBlock) {
                    unsub()
                    const events = filterAndDecodeContractEvents(
                        result,
                        contract.abi,
                        logger
                    )
                    ...
                    logger.debug(
                        'extrinsicSuccess',
                        JSON.stringify(item.toHuman(), null, 4)
                    )
                    logger.debug('block number', result.blockNumber?.toString())
                    logger.debug('events', events)
                    const balance = await contract.api.query.system.account(
                        pair.address
                    )
                    logger.info(
                        'Sender balance After',
                        balance.data.free
                            .div(oneUnit(contract.api as ApiPromise))
                            .toString(),
                        'UNIT'
                    )
                    resolve()
