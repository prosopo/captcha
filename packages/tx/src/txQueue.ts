...
export class TransactionQueue {
	...
	public add(
		...
		try {
			this.queue.push({
				method: method || extrinsic.method.method.toString(),
				extrinsic,
				callback,
				pair,
			});
	...
	private async submit() {
		try {
			this.logger.debug(
				`TxQueue state: ${this.queue.length} items in the queue`,
			);
					...
					const unsub = await extrinsic.signAndSend(
						...
						async (result: SubmittableResult) => {
							...
							if (result.status.isInBlock || result.status.isFinalized) {
								...
								reject(
									new ProsopoTxQueueError(
										new Error("CONTRACT.TX_QUEUE_ERROR"),
										{
								...
								this.logger.debug(
									"Transaction is scheduled for a future block.",
								);
								reject(
									new ProsopoTxQueueError(
										new Error("CONTRACT.TX_QUEUE_ERROR"),
