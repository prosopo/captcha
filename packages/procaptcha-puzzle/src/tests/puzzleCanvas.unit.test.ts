...
describe("what it puts on screen", () => {
	...
	test("a retry adds the prompt without dropping the instruction", () => {
		...
		expect(overlay().textContent).toContain("Incorrect, please try again");
		expect(overlay().textContent).toContain("Drag the piece to the target");
	});

	test("the retry prompt is announced to assistive tech", () => {
		render(props({ showRetry: true }));
		const banner = overlay().querySelector('[data-cy="prosopo-retry-banner"]');
	...
	test("no prompt on the first go", () => {
		render(props());
		expect(
			overlay().querySelector('[data-cy="prosopo-retry-banner"]'),
		).toBeNull();
		expect(overlay().textContent).not.toContain("Incorrect");
	});
	...
	test("the prompt renders the message it is given", () => {
		...
		expect(overlay().textContent).toContain("Faux — réessayez");
	});
