...
/**
 * A minimal mount/unmount harness. The suites deliberately drive real DOM
 * events rather than calling handlers directly, because the checkbox drops
 * untrusted events — behaviour that only shows up when a real event carries
 * the flag.
 */
...
const setTrusted = (event: Event, trusted: boolean): void => {
	for (const symbol of Object.getOwnPropertySymbols(event)) {
		...
		if (impl && typeof impl === "object" && "isTrusted" in impl) {
			// A plain assignment would not survive: dispatchEvent stamps isTrusted
			// back to false on the way in, so the flag is pinned with an accessor
			// that swallows the write.
...
/**
 * Like `fire`, but hands back the event so a test can inspect what the handler
 * did to it — whether it prevented the default form submission, say.
 */
...
/** Colours land in the DOM as `rgb(...)`, so expectations convert hex first. */
