...
export class ProsopoServer {
	...
	public async verifyProvider(
		...
		this.logger.info({ providerUrl }, `Verifying with provider`);
		...
		if (challenge) {
			...
			if (!recent) {
				this.logger.error({ timestamp }, "PoW captcha is not recent");
				return {
					verified: false,
		...
		if (!recent) {
			this.logger.error({ timestamp }, "Image captcha is not recent");
			return {
				verified: false,
				status: i18n.t("API.USER_NOT_VERIFIED_TIME_EXPIRED"),
			};
	...
	public async isVerified(
		...
		try {
			...
			if (!provider) {
				this.logger.error({ providerUrl }, "Provider not found");
				return {
					verified: false,
