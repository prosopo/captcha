...
describe("connectToRedis", () => {
	...
	test("logs connection attempt with url", () => {
		...
		expect(logData?.data?.url).toBe(url);
	});
	...
	test("logs successful connection with awaiting time", async () => {
		...
		expect(connectionLog?.data).toHaveProperty("awaitingTimeMs");
		expect(typeof connectionLog?.data?.awaitingTimeMs).toBe("number");
	});
...
describe("setupRedisIndex", () => {
	...
	test("logs index setup start", async () => {
		...
		expect(setupLog?.data?.name).toBe("test-index");
	});
	...
	test("logs index setup completion with awaiting time", async () => {
		...
		expect(completionLog?.data?.name).toBe("test-index");
		expect(completionLog?.data).toHaveProperty("awaitingTimeMs");
		expect(typeof completionLog?.data?.awaitingTimeMs).toBe("number");
	});
