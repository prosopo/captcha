...
describe('CONTRACT TASKS', () => {
  ...
  it('Provider registration', async () => {
    ...
    it('Validate provided captcha dataset', async () => {
        ...
        const res = await tasks.getRandomProvider(dappUser.address, dapp.contractAccount);
        const blockNumberParsed = parseBlockNumber(res.block_number);
        const valid = await tasks.validateProviderWasRandomlyChosen(dappUser.address, dapp.contractAccount, res.provider.captcha_dataset_id as string, blockNumberParsed)
