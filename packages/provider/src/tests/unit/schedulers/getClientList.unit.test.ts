...
vi.mock("@prosopo/env", async () => {
	...
	const loggerOuter = getLogger("info", "test:get-client-list");
	const mockLogger = {
...
describe("getClientList", () => {
	...
	let mockTasks: {
		clientTaskManager: {
			getClientList: ReturnType<typeof vi.fn>;
		};
	};
	let mockCronJob: {
	...
	it("creates ProviderEnvironment and waits for readiness", async () => {
		await getClientList(mockPair, "0 0 * * *", mockConfig);
	...
	it("executes getClientList when task is not running", async () => {
		await getClientList(mockPair, "0 0 * * *", mockConfig);
		...
		expect(mockTasks.clientTaskManager.getClientList).toHaveBeenCalled();
	});

	it("does not execute getClientList when task is already running", async () => {
		...
		expect(mockTasks.clientTaskManager.getClientList).not.toHaveBeenCalled();
	});

	it("handles errors in getClientList", async () => {
		const error = new Error("Test error");
		mockTasks.clientTaskManager.getClientList.mockRejectedValue(error);
