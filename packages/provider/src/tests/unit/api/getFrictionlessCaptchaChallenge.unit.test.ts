...
describe("getFrictionlessCaptchaChallenge - access policy blocking", () => {
	// biome-ignore lint/suspicious/noExplicitAny: mock request
	const handler = getHandler({} as any, {} as any);
	...
	it("returns 403 ProsopoApiError when user has Block access policy", async () => {
		// Import AccessPolicyType for the test
		const { AccessPolicyType } = await import("@prosopo/user-access-policy");
		const { ProsopoApiError } = await import("@prosopo/common");

		// Arrange: client record with valid settings
		const clientRecord = {
			account: "site-blocked",
			settings: {
				contextAware: {
					enabled: true,
					contexts: {
		...
		// Mock decryptPayload to return valid data
		tasksInstance.frictionlessManager.decryptPayload.mockResolvedValue({
			baseBotScore: 0,
			timestamp: Date.now(),
			providerSelectEntropy: 0,
			...
			webView: false,
			iFrame: false,
			decryptedHeadHash: "abc123",
			decryptionFailed: false,
		});

		// Mock getPrioritisedAccessPolicies to return a Block policy
		const blockPolicy = {
		...
		const { req, res, next } = buildReqRes(body, "192.168.1.100");
		...
		expect(errorArg.translationKey).toBe("API.FORBIDDEN");
		expect(errorArg.code).toBe(403);
		expect(errorArg.context).toMatchObject({
			code: 403,
			siteKey: "site-blocked",
			user: "blocked-user",
		});

		// Verify that getPrioritisedAccessPolicies was called
		expect(
			tasksInstance.frictionlessManager.getPrioritisedAccessPolicies,
		).toHaveBeenCalledTimes(1);

		// Verify that sendImageCaptcha and sendPowCaptcha were NOT called
		expect(
	...
	it("continues normally when user has Restrict access policy", async () => {
		// Import AccessPolicyType for the test
		const { AccessPolicyType } = await import("@prosopo/user-access-policy");

		// Arrange: client record with valid settings
		const clientRecord = {
			account: "site-restricted",
			settings: {
				contextAware: {
					enabled: true,
					contexts: {
		...
		// Mock getPrioritisedAccessPolicies to return a Restrict policy
		const restrictPolicy = {
			type: AccessPolicyType.Restrict,
			description: "User is restricted",
			frictionlessScore: 0.8,
		};
		tasksInstance.frictionlessManager.getPrioritisedAccessPolicies.mockResolvedValue(
			[restrictPolicy],
		);

		tasksInstance.frictionlessManager.getClientContextEntropy.mockResolvedValue(
			"entropy-123",
		);

		const body = {
			token: "restricted-token",
			headHash: "hh-restricted",
			dapp: "site-restricted",
			user: "restricted-user",
		};
		const { req, res, next } = buildReqRes(body, "192.168.1.200");

		// Act
		// biome-ignore lint/suspicious/noExplicitAny: mock request
		await handler(req as any, res as any, next);

		// Assert: next should NOT be called with an error (processing continues)
		// Instead, the handler should continue to send a captcha response
		expect(
			tasksInstance.frictionlessManager.getPrioritisedAccessPolicies,
		).toHaveBeenCalled();

		// The handler should continue processing (not blocked)
		// Either sendImageCaptcha or sendPowCaptcha should eventually be called
		// depending on the score calculation
	});
});
