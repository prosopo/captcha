...
describe("IP Validation Integration Tests", () => {
	...
	describe("Frictionless captcha with IP validation", () => {
		...
		it("should reject image captcha request from different IP after frictionless flow", async () => {
			const origin = "http://localhost";
			const initialIP = "127.0.0.1";
			const differentIP = "192.168.1.100";

			// Step 1: Get frictionless captcha from initial IP
			const getFrictionlessCaptchaUrl = `${baseUrl}${ClientApiPaths.GetFrictionlessCaptchaChallenge}`;
			const getFrictionlessCaptchaBody: GetFrictionlessCaptchaChallengeRequestBodyOutput =
				{
					[ApiParams.dapp]: siteKey,
			...
			// Step 2: Request image captcha from different IP
			const getImageCaptchaURL = `${baseUrl}${ClientApiPaths.GetImageCaptchaChallenge}`;
			const getImgCaptchaBody: CaptchaRequestBodyType = {
				[ApiParams.dapp]: siteKey,
				[ApiParams.user]: userId,
			...
			const responseImage = await fetch(getImageCaptchaURL, {
				...
				headers: {
					...
					"Prosopo-Site-Key": siteKey,
					"Prosopo-User": userId,
					"X-Forwarded-For": differentIP, // Different IP
				},
			});

			expect(responseImage.status).toBe(400);
			const errorData = await responseImage.json();
			expect(errorData.error).toContain("IP address mismatch");
		});

		it("should allow PoW captcha request from same IP after frictionless flow", async () => {
			const origin = "http://localhost";
			const initialIP = "127.0.0.1";
			...
			// Step 2: Request PoW captcha from same IP
			...
			const dataPoW = await responsePoW.json();
			expect(dataPoW).toHaveProperty("challenge");
		});

		it("should reject PoW captcha request from different IP after frictionless flow", async () => {
			const origin = "http://localhost";
			const initialIP = "127.0.0.1";
			const differentIP = "192.168.1.100";
			...
			// Step 2: Request PoW captcha from different IP
			const getPoWCaptchaUrl = `${baseUrl}${ClientApiPaths.GetPowCaptchaChallenge}`;
			const getPoWCaptchaBody: GetPowCaptchaChallengeRequestBodyTypeOutput = {
				[ApiParams.dapp]: siteKey,
				[ApiParams.user]: userId,
			...
			const errorData = await responsePoW.json();
			expect(errorData.error).toContain("IP address mismatch");
		});
	});
});