...
import {
	...
	type SubmitAudioCaptchaSolutionBodyTypeOutput,
} from "@prosopo/types";
...
export default (env: ProviderEnvironment) =>
	async (
		...
		if (getMaintenanceMode()) {
			...
			const response: AudioCaptchaSolutionResponse = {
				status: "ok",
				verified: true,
			};
		...
		let parsed: SubmitAudioCaptchaSolutionBodyTypeOutput;
		const tasks = new Tasks(env, req.logger);

		try {
			parsed = SubmitAudioCaptchaSolutionBody.parse(req.body);
		} catch (err) {
			return next(
		...
		try {
			...
			const verified =
				await tasks.audioCaptchaManager.verifyAudioCaptchaSolution(
					challenge,
			...
			req.logger.error(() => ({
				err,
				body: req.body,
				msg: "Error in audio captcha solution submission",
			}));
			return next(
