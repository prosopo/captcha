...
import {
	...
	type SubmitIconOrderCaptchaSolutionBodyTypeOutput,
} from "@prosopo/types";
...
export default (env: ProviderEnvironment) =>
	async (
		...
		if (getMaintenanceMode()) {
			...
			const response: IconOrderCaptchaSolutionResponse = {
				status: "ok",
				verified: true,
			};
		...
		let parsed: SubmitIconOrderCaptchaSolutionBodyTypeOutput;
		const tasks = new Tasks(env, req.logger);

		try {
			parsed = SubmitIconOrderCaptchaSolutionBody.parse(req.body);
		} catch (err) {
			return next(
		...
		try {
			...
			const verified =
				await tasks.iconOrderCaptchaManager.verifyIconOrderCaptchaSolution(
					challenge,
			...
			req.logger.error(() => ({
				err,
				body: req.body,
				msg: "Error in icon-order captcha solution submission",
			}));
			return next(
