...
import {
	...
	type SubmitConnectCaptchaSolutionBodyTypeOutput,
} from "@prosopo/types";
...
export default (env: ProviderEnvironment) =>
	async (
		...
		if (getMaintenanceMode()) {
			...
			const response: ConnectCaptchaSolutionResponse = {
				status: "ok",
				verified: true,
			};
		...
		let parsed: SubmitConnectCaptchaSolutionBodyTypeOutput;
		const tasks = new Tasks(env, req.logger);

		try {
			parsed = SubmitConnectCaptchaSolutionBody.parse(req.body);
		} catch (err) {
			return next(
		...
		try {
			...
			const verified =
				await tasks.connectCaptchaManager.verifyConnectCaptchaSolution(
					challenge,
			...
			req.logger.error(() => ({
				err,
				body: req.body,
				msg: "Error in connect captcha solution submission",
			}));
			return next(
