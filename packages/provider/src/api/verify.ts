...
import {
	...
	ServerConnectCaptchaVerifyRequestBody,
	type ServerConnectCaptchaVerifyRequestBodyOutput,
...
const VERIFY_PATH_TYPE: Partial<Record<ClientApiPaths, CaptchaType>> = {
	...
	[ClientApiPaths.VerifyConnectCaptchaSolution]: CaptchaType.connect,
};
...
export function prosopoVerifyRouter(env: ProviderEnvironment): Router {
	...
	/**
	 * Verifies a dapp's connect captcha solution as being approved or not
	 *
	 * @param {string} token - Token containing dapp, blockNumber and challenge
	 * @param {string} dappSignature - Signed token
	 * @param {number} verifiedTimeout - The maximum time in milliseconds to be valid
	...
	router.post(
		...
		async (req, res, next) => {
			...
			if (getMaintenanceMode()) {
				req.logger.info(() => ({
					msg: "Maintenance mode active - returning verified for connect captcha verification",
				}));
				const verificationResponse: VerificationResponse =
					buildMaintenanceVerificationResponse(req.i18n.t);
				return res.json(verificationResponse);
			...
			let parsed: ServerConnectCaptchaVerifyRequestBodyOutput;

			// We can be helpful and provide a more detailed error message when there are missing fields
			try {
				parsed = ServerConnectCaptchaVerifyRequestBody.parse(req.body);
			} catch (err) {
				return next(
					new ProsopoApiError("CAPTCHA.PARSE_ERROR", {
						context: { code: 400, error: err, body: req.body },
			...
			try {
				...
				const forwarded = await forwardVerifyIfNotIssuer({
					env,
					logger: req.logger,
					path: ClientApiPaths.VerifyConnectCaptchaSolution,
					providerUrl,
					dapp,
					user,
					body: parsed,
				...
				const { verified, score } =
					await tasks.connectCaptchaManager.serverVerifyConnectCaptchaSolution(
						dapp,
						challenge,
						clientRecord.settings.verifiedTimeout,
						env,
				...
				const verificationResponse: VerificationResponse =
					tasks.connectCaptchaManager.getVerificationResponse(
						verified,
						clientRecord,
						req.i18n.t,
						score,
				...
				req.logger.error(() => ({
					msg: "Error in verifyConnectCaptchaSolution",
					err,
					data: { body: req.body },
				}));
				return next(
