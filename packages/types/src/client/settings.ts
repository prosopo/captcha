...
// Floor on image rounds. Matches the historical hard-coded floor (the
// provider's `captchas.solved.count` default and the `Math.max(2, …)` that
// used to live in `timestampDecayFunction`), so leaving it unset preserves
// existing behaviour.
