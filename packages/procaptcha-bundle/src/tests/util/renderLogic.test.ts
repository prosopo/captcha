...
import { mountProcaptchaFrictionless } from "@prosopo/procaptcha-frictionless";
...
vi.mock("@prosopo/procaptcha-frictionless", async () => {
	return {
		mountProcaptchaFrictionless: vi.fn(
			(container: HTMLElement, _props: ProcaptchaProps) => {
				const element = container.ownerDocument.createElement("div");
				element.textContent = "ProcaptchaFrictionless";
...
describe("Config utility functions", () => {
	...
	it<TestContext>("always mounts the unified frictionless dispatcher", async (ctx) => {
		...
		await vi.waitFor(() => {
			expect(mountProcaptchaFrictionless).toHaveBeenCalledTimes(1);
		});
	});
	...
	it<TestContext>("user callbacks should be set when defined on the window and pulled in via data attributes", async (ctx) => {
		...
		await vi.waitFor(() => {
			...
			// @ts-ignore
			const call = mountProcaptchaFrictionless.mock.calls[0];
			const props = call[1];
