...
// export type Unpack<T extends Pack<any>> = {
//     // required + readwrite keys
//     [K in keyof T as IsRequired<T[K]> extends true ? IsReadWrite<T[K]> extends true ? K : never : never]: T[K] extends Parser<infer U> ? U : T[K] extends Pack<any> ? Unpack<T[K]> : never
// } & {
//     // optional + readwrite keys
//     [K in keyof T as IsOptional<T[K]> extends true ? IsReadWrite<T[K]> extends true ? K : never : never]?: T[K] extends Parser<infer U> ? U : T[K] extends Pack<any> ? Unpack<T[K]> : never
// } & {
//     // required + readonly keys
//     readonly [K in keyof T as IsRequired<T[K]> extends true ? IsReadonly<T[K]> extends true ? K : never : never]: T[K] extends Parser<infer U> ? U : T[K] extends Pack<any> ? Unpack<T[K]> : never
// } & {
//     // optional + readonly keys
//     readonly [K in keyof T as IsOptional<T[K]> extends true ? IsReadonly<T[K]> extends true ? K : never : never]?: T[K] extends Parser<infer U> ? U : T[K] extends Pack<any> ? Unpack<T[K]> : never
// }
