...
describe("i18nBackend", () => {
	beforeEach(() => {
		vi.resetModules();
		vi.clearAllMocks();
	});

	test("should initialize i18n when not already initialized and on server side", async () => {
		const mockI18n = {
			isInitialized: false,
		...
		const { initializeI18n } = await import("../i18nBackend.js");
	...
	test("should not initialize when on client side", async () => {
		const mockI18n = {
			isInitialized: false,
			use: vi.fn().mockReturnThis(),
			init: vi.fn(),
