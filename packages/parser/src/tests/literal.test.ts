...
describe("literal", () => {
    ...
    test("native enum", () => {
        enum Foo { A, B, C }
        expect(pLiteral(Foo.A).parse(Foo.A)).toEqual(Foo.A)
    })

    test("enum", () => {
        const values = ["A", "B", "C"] as const
        expect(pLiteral(values[0]).parse(values[0])).toEqual(values[0])
    })

    test("type", () => {
        assertType<1>(pLiteral(1).parse(1))
        assertType<true>(pLiteral(true).parse(true))
        assertType<"a">(pLiteral("a").parse("a"))
    })

    test("tuple", () => {
        expect(() => pLiteral(1).parse(["a", 1, true])).toThrow()
    })
})