...
describe("boolean", () => {
    ...
    test("native enum", () => {
        enum Foo { A, B, C }
        expect(() => pBoolean().parse(Foo.A)).toThrow()
    })

    test("enum", () => {
        const values = ["A", "B", "C"] as const
        expect(() => pBoolean().parse(values)).toThrow()
        expect(() => pBoolean().parse(values[0])).toThrow()
    })

    test("type", () => {
        assertType<boolean>(pBoolean().parse(true))
    })

    test("tuple", () => {
        expect(() => pBoolean().parse(["a", 1, true])).toThrow()
    })
})