...
import { u64 } from '../parsers/U64Parser.js';
import { u128 } from '../parsers/U128Parser.js';
import { i256 } from '../parsers/I256Parser.js';
...
const p = i256
const min = (BigInt(2) ** BigInt(256)) / 2n - (BigInt(2) ** BigInt(256))
const max = (BigInt(2) ** BigInt(256)) / 2n - 1n

describe("u64", () => {
    it("should have correct typename", () => {
        expect(p().name).toBe(`i256`)
    })
    
    it(`should error on < ${min}`, () => {
        expect(() => p().validate(min - 1n)).toThrow();
    })

    it(`should error on > ${max}`, () => {
        expect(() => p().validate(max + 1n)).toThrow();
    })

    it(`should parse ${min} - ${max}`, () => {
        expect(p().validate(min).toString()).toBe(min.toString());
        expect(p().validate(max).toString()).toBe(max.toString());
    })

    it("should parse to correct type", () => {
        expectTypeOf(() => p().validate(null)).returns.toMatchTypeOf<bigint>();
        const parser = p();
        const a: bigint = 1n;
    ...
    it("should parse BigInt", () => {
        expect(p().validate(BigInt(1))).toBe(1n);
    })
