...
describe("map", () => {
    ...
    it("should parse to correct type", () => {
        expectTypeOf(() => p().shape(null)).returns.toMatchTypeOf<Map<string, number>>();
        const parser = p();
    ...
    it("should parse populated Map", () => {
        expect(p().shape(new Map([["a", 1], ["b", 2], ["c", 3]]))).toStrictEqual(new Map([["a", 1], ["b", 2], ["c", 3]]));
    })

    it("should error on Map with invalid key type", () => {
        expect(() => p().shape(new Map<string | number, number>([[1, 1], ["b", 2], ["c", 3]]) as any)).toThrow();
    })

    it("should error on Map with invalid value type", () => {
        expect(() => p().shape(new Map<string, string | number>([["a", "1"], ["b", 2], ["c", 3]]) as any)).toThrow();
    })

    it("should parse Map", () => {
        expect(p().shape(new Map())).toStrictEqual(new Map());
    })
