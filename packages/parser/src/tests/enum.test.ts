...
describe("enum", () => {
    ...
    it("should parse to correct type", () => {
        ...
        expectTypeOf(() => en(variants).shape(null)).returns.toMatchTypeOf<typeof variants[number]>();
    ...
    it("should parse variant", () => {
        expect(p().shape('a')).toBe('a');
        expect(p().shape('b')).toBe('b');
        expect(p().shape('c')).toBe('c');
    })

    it("should error on invalid variant", () => {
        expect(() => p().shape('d')).toThrow();
    })
