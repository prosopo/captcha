...
describe("instance", () => {
    ...
    it("should parse to correct type", () => {
        expectTypeOf(() => p().validate(null)).returns.toMatchTypeOf<Bar>();
        expectTypeOf(() => inst(Baz).validate(null)).returns.toEqualTypeOf<Baz>();
        const parser = p();
        const a: Bar = new Bar();
        const b: ReturnType<typeof parser.validate> = a;
        const parser2 = inst(Baz);
        const c: Baz = new Baz();
        const d: ReturnType<typeof parser2.validate> = c;
    })
    ...
    it("should parse instance", () => {
        expect(p().validate(new Bar())).toEqual(new Bar());
        expect(p().validate(new Bar())).toBeInstanceOf(Bar);
    })
