...
describe("instance", () => {
    ...
    it("should parse to correct type", () => {
        expectTypeOf(() => p().shape(null)).returns.toMatchTypeOf<Bar>();
        expectTypeOf(() => inst(Baz).shape(null)).returns.toEqualTypeOf<Baz>();
        const parser = p();
        const a: Bar = new Bar();
        const b: ReturnType<typeof parser.shape> = a;
        const parser2 = inst(Baz);
        const c: Baz = new Baz();
        const d: ReturnType<typeof parser2.shape> = c;
    })
    ...
    it("should parse instance", () => {
        expect(p().shape(new Bar())).toEqual(new Bar());
        expect(p().shape(new Bar())).toBeInstanceOf(Bar);
    })
