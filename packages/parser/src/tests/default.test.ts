...
describe("default", () => {
    ...
    it("should parse to correct type", () => {
        expectTypeOf(() => p().shape(null)).returns.toMatchTypeOf<string>();
        const parser = p();
        const a: string = 'a';
    ...
    it("should default on undefined", () => {
        expect(p().shape(undefined)).toBe('xyz');
    })
    ...
    it("should error on native enum variant", () => {
        expect(p().shape(Foo.A)).toEqual('x');
        expect(() => p().shape(Foo.B)).toThrow();
        expect(() => p().shape(Foo.C)).toThrow();
        expect(p().shape(Foo.D)).toEqual('y');
    })
