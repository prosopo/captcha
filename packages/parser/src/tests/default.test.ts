...
import { def } from '../parsers/DefaultParser.js';
...
describe("default", () => {
    it("should have correct typename", () => {
        expect(p().name).toBe(`${p().validator.name} = xyz`)
        expect(p().name).toBe(`string = xyz`)
    })
    
    it("should parse to correct type", () => {
        expectTypeOf(() => p().validate(null)).returns.toMatchTypeOf<string>();
        const parser = p();
        const a: string = 'a';
    ...
    it("should parse string", () => {
        expect(p().validate("abc")).toBe("abc");
    })
    ...
    it("should default on undefined", () => {
        expect(p().validate(undefined)).toBe('xyz');
    })
    ...
    it("should error on native enum variant", () => {
        expect(p().validate(Foo.A)).toEqual('x');
        expect(() => p().validate(Foo.B)).toThrow();
        expect(() => p().validate(Foo.C)).toThrow();
        expect(p().validate(Foo.D)).toEqual('y');
    })
