...
describe("native enum", () => {
    ...
    it("should parse to correct type", () => {
        expectTypeOf(() => p().validate(null)).returns.toMatchTypeOf<Foo>();
        const parser = p();
        const a: Foo = Foo.A;
        ...
        const g: Foo = Foo.D;
        const h: ReturnType<typeof parser.validate> = g;
    })
    ...
    it("should parse variant", () => {
        expect(p().validate('x')).toBe(Foo.A);
        expect(p().validate(2)).toBe(Foo.B);
        expect(p().validate(3)).toBe(Foo.C);
        expect(p().validate('y')).toBe(Foo.D);
    })

    it("should error on invalid variant", () => {
        expect(() => p().validate('z')).toThrow();
