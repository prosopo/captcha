...
export class SchemaHandler<T extends Schema<any>> {
    ...
    public extend<U>(schema: Schema<U>): Schema<Extend<T, U>> {
        ...
        for (const key in this._schema) {
            result[key] = get(this._schema, key).clone()
        }
...
export class ObjectParser<T extends Schema<any>> extends Parser<UnpackSchema<T>> {
    ...
    public pick<U extends Mask<T>>(mask: U) {
        return new ObjectParser<Schema<DeepPick<T, U>>>(new SchemaHandler(this.schema).pick(mask))
    }

    public omit<U extends Mask<T>>(mask: U) {
        return new ObjectParser<Schema<DeepOmit<T, U>>>(new SchemaHandler(this.schema).omit(mask))
    }

    public extend<U>(schema: Schema<U>) {
        return new ObjectParser<Schema<Extend<T, U>>>(new SchemaHandler(this.schema).extend(schema))
    }

    public merge<U>(parser: ObjectParser<Schema<U>>) {
        // extend the schema, then wrap it in a new object parser
        return new ObjectParser<Schema<Extend<T, U>>>(new SchemaHandler(this.schema).extend(parser.schema))
    }
}

export const pObject = <U extends Schema<any>>(schema: U) => new ObjectParser<U>(schema)
