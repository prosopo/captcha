...
import { type Logger, LogLevel } from "@prosopo/logger";
...
interface Harness {
	...
	request: Request;
	response: Response;
	next: ReturnType<typeof vi.fn<NextFunction>>;
	logger: Logger;
...
describe("a request the endpoint accepts", () => {
	...
	test("passes undefined args when the endpoint declares no schema", async () => {
		...
		expect(target.processRequest).toHaveBeenCalledWith(undefined, harness.logger);
	});
	...
	test("answers with the endpoint's response as JSON", async () => {
		await harness.adapter.handleRequest(
			endpoint(undefined, async () => ({ status: "ok", value: 1 })),
			harness.request,
			harness.response,
			harness.next,
		);
		expect(harness.json).toHaveBeenCalledWith({ status: "ok", value: 1 });
		expect(harness.next).not.toHaveBeenCalled();
	});

	test("stringifies bigints, which res.json() cannot serialise", async () => {
		await harness.adapter.handleRequest(
			endpoint(undefined, async () => ({ status: "ok", big: 10n })),
			harness.request,
			harness.response,
			harness.next,
		);
		expect(harness.json).toHaveBeenCalledWith({ status: "ok", big: "10" });
	});

	test("passes a null or empty response straight through", async () => {
	...
	test("an absent request body is still handed to the schema", async () => {
		...
		expect(harness.json).toHaveBeenCalledWith({ status: "ok" });
	});
});
...
describe("a body the schema rejects", () => {
	...
	test("is reported as a 400 parse error carrying the cause", async () => {
		...
		const [forwarded] = harness.next.mock.calls[0] ?? [];
	...
	test("a schema that throws something other than a ZodError is treated the same", async () => {
		...
		expect(harness.next).toHaveBeenCalledTimes(1);
		expect(harness.next.mock.calls[0]?.[0]).toBeInstanceOf(ProsopoApiError);
	});
	...
	test("an endpoint that fails while building its schema is caught too", async () => {
		...
		const target: ApiEndpoint<ZodType | undefined> = {
			...
			processRequest: vi.fn<
				ApiEndpoint<ZodType | undefined>["processRequest"]
			>(),
		};
