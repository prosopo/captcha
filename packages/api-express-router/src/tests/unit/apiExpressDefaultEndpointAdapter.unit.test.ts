...
describe("ApiExpressDefaultEndpointAdapter", () => {
    ...
    describe("type tests", () => {
        ...
        it("should have correct handleRequest signature", () => {
            ...
            expectTypeOf(adapter.handleRequest).parameter(0).toMatchTypeOf<ApiEndpoint<z.ZodType | undefined>>();
            expectTypeOf(adapter.handleRequest).parameter(1).toMatchTypeOf<Request>();
            expectTypeOf(adapter.handleRequest).parameter(2).toMatchTypeOf<Response>();
            expectTypeOf(adapter.handleRequest).parameter(3).toMatchTypeOf<NextFunction>();

            // Type test: verify return type
            expectTypeOf(adapter.handleRequest).returns.toMatchTypeOf<Promise<void>>();
        });
    });
});
