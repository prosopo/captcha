...
import type { IncomingHttpHeaders, NextFunction, Request, Response } from "express";
...
vi.mock("@prosopo/logger", async () => {
	const actual = await vi.importActual<typeof import("@prosopo/logger")>(
		"@prosopo/logger",
	);
...
interface Harness {
	...
	setHeader: ReturnType<typeof vi.fn<(name: string, value: string) => Response>>;
}
...
const build = (
	...
	return { req, res, next: vi.fn<NextFunction>(), setHeader };
};

const run = (harness: Harness): void => {
	requestLoggerMiddleware(env)(harness.req, harness.res, harness.next);
};
...
describe("envelope logging", () => {
	...
	test("logs the response only once when both events fire", () => {
		...
		harness.res.emit("finish");
		harness.res.emit("close");
		expect(recorded.infos.filter((e) => e.msg === "Response sent")).toHaveLength(
			1,
		);
	});
