...
async function generateAllShapesTest(baseArgs: any): Promise<void> {
	...
	console.log(
		`Generating test dataset with one captcha for each of the ${shapeNames.length} shapes...`,
	);
	...
	if (images.length === 0) {
		console.error(
			`No images found in ${baseImageDir}. Please add some images or use --auto-generate.`,
		);
		process.exit(1);
	}
...
async function main() {
	...
	const argv = yargs(hideBin(process.argv))
		.command(
			"generate",
			"Generate a slider captcha dataset",
			() => {},
			(argv) => {
	...
	try {
		...
		console.log("---------------------------------------");
		console.log(
			`Source images: ${baseImageDir} (${images.length} images found)`,
		);
		...
		const baseImages = assetFiles.filter((file) =>
			file.startsWith("base_"),
		).length;
		const pieceImages = assetFiles.filter((file) =>
			file.startsWith("piece_"),
		).length;

		console.log(
			`Generated ${baseImages} base images and ${pieceImages} puzzle pieces`,
		);
		...
		if (verbose) {
			...
			console.log(
				"- Puzzle pieces: PNG files with shapes extracted from base images in assets/",
			);
			console.log(
				"- Each puzzle piece has a target position recorded in the dataset.json",
			);
			console.log(
				"- Users must position the piece within tolerance to solve the captcha",
			);
			console.log("\nAvailable shapes:");
		...
		console.log(
			"2. Update your provider's config.json to include the dataset ID",
		);
		console.log(
			"3. Ensure the assets directory is accessible to your provider",
		);
	} catch (error) {
