...
async function setupShapedSliderCaptchas(options: {
	...
	if (!existsSync(providerDir)) {
		...
		console.warn(
			"Will continue with dataset generation but provider setup may fail.",
		);
	}
	...
	try {
		...
		console.error(
			"Setup will continue but provider configuration may need to be done manually.",
		);
	}
	...
	if (configUpdated) {
		...
		console.warn(
			'  1. Add "slider": { "useShapedCaptchas": true } to your config',
		);
