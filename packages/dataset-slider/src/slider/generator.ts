...
export class SliderDatasetGenerator {
	...
	private getShape(): PuzzleShape {
		...
		if (selectedShapeName) {
			...
			console.warn(
				`Shape "${selectedShapeName}" not found. Using a random shape instead.`,
			);
		}
	...
	async generate(): Promise<void> {
		...
		try {
			...
			throw new Error(
				`Base image directory ${baseImageDir} does not exist or is not accessible`,
			);
		}
		...
		if (imageFiles.length === 0) {
			throw new Error(
				`No image files found in ${baseImageDir}. Please add some images.`,
			);
		}
		...
		console.log(
			`Dataset saved to: ${resolve(join(outputDir, "dataset.json"))}`,
		);
	 ...
	 */
	private async createMaskFromSVG(
	...
	private async generateCaptcha(
		...
		try {
			...
			console.log(
				`Puzzle position: x=${puzzleX}, y=${puzzleY}, width=${finalWidth}, height=${finalHeight}`,
			);
			console.log(`Using puzzle shape: ${puzzleShape.name}`);

			// Create mask for the shape
			const maskBuffer = await this.createMaskFromSVG(
				puzzleShape,
				finalWidth,
				finalHeight,
			);
			...
			await sharp(pieceBuffer).png().toFile(pieceImagePath);
			...
			// Save the base image
			await sharp(baseBuffer).png().toFile(baseImagePath);
