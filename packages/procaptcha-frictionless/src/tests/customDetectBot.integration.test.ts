...
describe("customDetectBot", () => {
	...
	/**
	 * Tests that the function validates the provider selection
	 * and throws appropriate error when no provider is found
	 */
	it("should throw error when detector returns no provider", async () => {
		class MockExtension {
			getAccount = vi.fn().mockResolvedValue(mockUserAccount);
		}
	...
	/**
	 * Tests that the function handles different web2 configurations
	 * and passes the correct web2 flag to ExtensionLoader
	 */
	it("should handle web2 false configuration", async () => {
		const configWeb2False = { ...mockConfig, web2: false };
		...
		await customDetectBot(configWeb2False, mockContainer, mockRestartFn);

		expect(ExtensionLoader).toHaveBeenCalledWith(false);
	});

	/**
	 * Tests that the function properly handles and propagates
	 * errors from the ExtensionLoader
	 */
	it("should handle extension loading errors", async () => {
		const extensionError = new Error("Extension loading failed");
		vi.mocked(ExtensionLoader).mockRejectedValue(extensionError);
	...
	/**
	 * Tests that the function properly handles and propagates
	 * errors from the DetectorLoader
	 */
	it("should handle detector loading errors", async () => {
		...
		const detectorError = new Error("Detector loading failed");
		vi.mocked(DetectorLoader).mockRejectedValue(detectorError);

		await expect(
			customDetectBot(mockConfig, mockContainer, mockRestartFn),
		).rejects.toThrow("Detector loading failed");
	});
});
