...
const validContextParameters = new Set([
	...
	36005, 36347, 36348, 36349, 37440, 37441, 37443, 7936, 7937,
	7938,
 ...
 */
export function getWebGlBasics({
	cache,
}: Options): WebGlBasicsPayload | SpecialStatus {
...
export function getWebGlExtensions({
	cache,
}: Options): WebGlExtensionsPayload | SpecialStatus {
	const gl = getWebGLContext(cache);
	if (!gl) {
		return STATUS_NO_GL_CONTEXT;
	}
