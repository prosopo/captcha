...
interface ConnectManagerHandle {
	start: (
		x?: number,
		y?: number,
	) => Promise<GetConnectCaptchaResponse | undefined>;
	submitSolution: (
		sourceIndex: number,
		targetIndex: number,
...
export const Manager = (
...
): ConnectManagerHandle => {
	const events = getDefaultEvents(callbacks);

	// Closure variables to share state between start and submitSolution
	let storedChallengeResponse: GetConnectCaptchaResponse | undefined;
	...
	const setValidChallengeTimeout = () => {
		const timeMillis: number = getConfig().captchas.connect.solutionTimeout;
		...
		await providerRetry(
			async () => {
				...
				const simdReadingsOnChallenge = frictionlessState?.getSimdReadings
					...
					: undefined;
				const challenge = await providerApi.getConnectCaptchaChallenge(
					userAccount,
					getDappAccount(),
				...
				// Set loading to false to signal the widget to show the board
				updateState({
					loading: false,
				});
			},
	...
	const submitSolution = async (
		...
		try {
			...
			// records the (x, y) on the connect captcha record for telemetry.
			...
			const verifiedSolution = await providerApi.submitConnectCaptchaSolution(
				challenge,
			...
			if (verifiedSolution[ApiParams.verified]) {
				...
				events.onHuman(
					encodeProcaptchaOutput({
						...
						[ApiParams.captchaType]: CaptchaType.connect,
					}),
				);
