...
describe("DeleteRulesEndpoint", () => {
	...
	it("should return success after deleting rules", async () => {
		const mockStorage: AccessRulesStorage = {
			fetchRules: vi.fn(),
			getMissingRuleIds: vi.fn(),
			findRules: vi.fn(),
			findRuleIds: vi.fn().mockResolvedValue(["id1", "id2"]),
		...
		const endpoint = new DeleteRulesEndpoint(mockStorage, mockLogger);
	...
	it("should handle multiple filter inputs", async () => {
		const mockStorage: AccessRulesStorage = {
			fetchRules: vi.fn(),
			getMissingRuleIds: vi.fn(),
			findRules: vi.fn(),
	...
	it("should log information when deleting rules", async () => {
		const mockStorage: AccessRulesStorage = {
			fetchRules: vi.fn(),
			getMissingRuleIds: vi.fn(),
			findRules: vi.fn(),
		...
		const endpoint = new DeleteRulesEndpoint(mockStorage, logger);
