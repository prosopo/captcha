...
const IMAGE_PROCESSING_TIMEOUT_MS = 120_000;

describe("dataset commands", () => {
	const fixturesDir = `${__dirname}/data`;
	// Where the fixtures' `${repo}` placeholder resolves to, i.e. how a
	// command's output paths must read to match the checked-in fixture.
	const fixturePathInRepo = (fixture: string): string =>
		path.relative(REPO_DIR, `${fixturesDir}/${fixture}`);
	let tmpDir: string;
	let testResultsDir: string;
	let resolvedFixturesDir: string;

	const resolvedFixture = (fixture: string): string =>
		substituteRepoDir(
			`${fixturesDir}/${fixture}`,
			`${resolvedFixturesDir}/${fixture}`,
		);

	// The command wrote absolute paths under `outputDir`; the fixture holds the
	// same files under `${repo}/<fixture dir>`.
	const expectDataJsonMatchesFixture = (
		outputDir: string,
		fixture: string,
	): void => {
		const actual = readJson(`${outputDir}/data.json`, [
			fs.realpathSync(outputDir),
			`${REPO_PLACEHOLDER}/${fixturePathInRepo(path.dirname(fixture))}`,
		]);
		expect(actual).toEqual(readJson(`${fixturesDir}/${fixture}`));
	};

	beforeAll(() => {
		tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "datasets-fs-test-"));
		testResultsDir = `${tmpDir}/results`;
		resolvedFixturesDir = `${tmpDir}/fixtures`;
	...
	test("labels", async () => {
		const input = resolvedFixture("flat_resized/data.json");
		...
		expect(readJson(output)).toEqual(
			readJson(`${fixturesDir}/flat_resized/labels.json`),
		);
		...
		const categories = fs.readdirSync(`${fixturesDir}/hierarchical`);
	...
	test(
		...
		async () => {
			...
			if (
				!captchasEqFs(output, resolvedFixture("flat_resized/captchas_v2.json"))
			) {
	...
	test("generate v1", async () => {
		...
		// make sure the results are the same as the expected results
		if (
			!captchasEqFs(output, resolvedFixture("flat_resized/captchas_v1.json"))
		) {
			throw new Error("captchas not equal");
		}
	});
	...
	test(
		"resizes data",
		async () => {
			const input = resolvedFixture("flat/data.json");
			...
			expect(
				fsEq(`${output}/images`, `${fixturesDir}/flat_resized/images`),
			).toBe(true);
			expectDataJsonMatchesFixture(output, "flat_resized/data.json");
	...
	test("relocates data", async () => {
		const input = `${fixturesDir}/flat_resized/data.json`;
		...
		await relocate.exec({
			...
			overwrite: true,
			to: "newwebsite.com",
			from: REPO_PLACEHOLDER,
		});

		expect(readJson(output)).toEqual(
			readJson(`${fixturesDir}/flat_resized/relocated_data.json`),
		);
	});
	...
	test(
		...
		async () => {
			const input = `${fixturesDir}/hierarchical`;
			...
			expect(fsEq(`${output}/images`, `${fixturesDir}/flat/images`)).toBe(true);
			expectDataJsonMatchesFixture(output, "flat/data.json");
