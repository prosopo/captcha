...
import { captchaContextReducer, CaptchaEventCallbacks, captchaStatusReducer, GetCaptchaResponse, ICaptchaContextState, ICaptchaState, ProsopoCaptchaClient, ProsopoCaptchaConfig, ProsopoCaptchaStateClient, TCaptchaSubmitResult } from "@prosopo/procaptcha";
...
export const Procaptcha = (props: ProcaptchaProps) => {
    ...
    const client = new ProsopoCaptchaClient({ state: state, update: updateState }, {
        onSolved: (result: TCaptchaSubmitResult, isHuman?: boolean) => {
            callbacks.onSolved?.(result, isHuman);
            setTicked(isHuman || false);
    ...
    return (
        <Box sx={{maxWidth: "100%", maxHeight: "100%", overflowX: "auto"}}>
            ...
            <Box p={1} sx={{maxWidth: "600px", minWidth: "200px"}}>
                <Box p={1} border={1} borderColor="grey.300" borderRadius={2} sx={{ display: 'flex', justifyContent: "space-between", alignItems: "center", flexWrap: 'wrap', }}>
                    ...
                    <Box  p={1} sx={{flexGrow: "1", maxWidth: "140px", minWidth: "140px"}}>
                        <Link href="https://prosopo.io" target="_blank">
                            <Box>
                                <div style={{width: "100%", display: "flex", alignItems: "middle"}} dangerouslySetInnerHTML={{__html: logo}}></div>
                            </Box>
                        </Link>
                    </Box>
                </Box>
