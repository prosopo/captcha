...
} from "../components/captchaComponent.js";
import { captcha, challengeResponse, translator } from "./harness.js";
import { type Mounted, asRgb, fire, mount } from "./render.js";

let mounted: Mounted;
let component: Component<CaptchaComponentProps> | undefined;
...
const render = (overrides: Parameters<typeof props>[0] = {}): void => {
	if (component) {
		component.update(props(overrides));
	} else {
		component = mountCaptchaComponent(mounted.container, props(overrides));
	}
};
...
describe("the controls", () => {
	...
	test("advancing a round swaps next for submit in place", () => {
		const challenge = twoRoundChallenge();
		render({ challenge, index: 0, solutions: [[], []] });
		const next = buttonLabelled("WIDGET.NEXT");
		render({ challenge, index: 1, solutions: [[], []] });
		expect(buttonLabelled("WIDGET.SUBMIT")).toBe(next);
	});
