...
// The effect that reports is keyed on the account object, so tests that
// re-render share one instance and only the tests about account changes pass a
// different one.
...
const render = (
	...
	mounted.render(
		<Collector
			...
			sendData={props.sendData ?? true}
			account={"account" in props ? props.account : defaultAccount}
		/>,
	);
};
...
describe("handing the events back", () => {
	...
	test("reports the events the collector has gathered so far", () => {
		...
		// A state change alone does not report — the effect is keyed on the
		// account, deliberately, so a re-render mid-collection is not a send.
		expect(onProcessData).toHaveBeenCalledTimes(1);
		render();
		expect(onProcessData).toHaveBeenCalledTimes(1);
