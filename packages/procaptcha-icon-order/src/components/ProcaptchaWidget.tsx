...
import { IconOrderCanvas } from "./IconOrderCanvas.js";
...
const Procaptcha = (props: ProcaptchaProps) => {
	...
	const [iconOrderPhase, setIconOrderPhase] =
		useState<IconOrderPhase>("checkbox");
	const [challengeData, setChallengeData] =
		useState<GetIconOrderCaptchaResponse | null>(null);
	...
	useEffect(() => {
		...
		if (config.mode === ModeEnum.invisible) {
			...
			// then drive the icon-order UI through the same phase transitions as
	...
	const handleIconOrderComplete = useCallback(
		async (clicks: IconClick[], events: IconOrderEvent[]) => {
			setIconOrderPhase("submitting");
			let verified = false;
			try {
				verified = await manager.current.submitSolution(clicks, events);
			} catch (error) {
				callbacks.onError?.(
					error instanceof Error ? error : new Error(String(error)),
				);
	...
	const showIconOrderOverlay =
		(iconOrderPhase === "selecting" || iconOrderPhase === "submitting") &&
		challengeData;
	...
	return (
		<>
			...
			{/* Icon-order overlay — rendered outside the shadow DOM flow via
			    fixed positioning. Shown in both visible and invisible modes
			    once a challenge has been fetched; selecting icons in order is
			    inherently interactive. */}
			{showIconOrderOverlay && (
				<IconOrderCanvas
					background={challengeData.background}
					legend={challengeData.legend}
					legendIconSize={challengeData.legendIconSize}
					onComplete={handleIconOrderComplete}
					showRetry={showRetry}
			...
			{!isInvisible && (
				<Checkbox
					...
					loading={loading || iconOrderPhase === "submitting"}
				/>
			)}
		</>
	);
