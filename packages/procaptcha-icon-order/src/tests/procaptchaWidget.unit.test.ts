...
// simulating a drag.
vi.mock("../components/IconOrderCanvas.js", async () => {
...
describe("starting from the checkbox", () => {
	...
	test("a second click while the first is in flight is ignored", async () => {
		...
		mocks.start.mockReturnValue(
			new Promise<GetIconOrderCaptchaResponse>((resolve) => {
				release = resolve;
			}),
		);
		render(props());
