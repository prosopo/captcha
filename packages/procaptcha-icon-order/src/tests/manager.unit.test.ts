...
const mocks = vi.hoisted(() => {
	...
	const getIconOrderCaptchaChallenge =
		vi.fn<
			(
				user: string,
				dapp: string,
		...
		>();
	const submitIconOrderCaptchaSolution =
		vi.fn<
			(
	...
	class ProviderApiMock {
		public getIconOrderCaptchaChallenge = getIconOrderCaptchaChallenge;
		public submitIconOrderCaptchaSolution = submitIconOrderCaptchaSolution;
	...
	return {
		...
		getIconOrderCaptchaChallenge,
		submitIconOrderCaptchaSolution,
		ProviderApiMock,
		getAccount,
		ExtensionMock,
		extensionLoader,
...
const submitArgs = (
	callIndex = 0,
): Parameters<typeof mocks.submitIconOrderCaptchaSolution> => {
	const call = mocks.submitIconOrderCaptchaSolution.mock.calls[callIndex];
...
beforeEach(() => {
	...
	mocks.getIconOrderCaptchaChallenge.mockResolvedValue(challengeResponse());
...
describe("start: the cases it refuses to run", () => {
	test("a solve already in flight is left alone", async () => {
		...
		expect(mocks.getIconOrderCaptchaChallenge).not.toHaveBeenCalled();
...
describe("start: getting to a challenge", () => {
	...
	test("passes the session id and prefetched SIMD readings to the challenge call", async () => {
		...
		await harness.manager.start();
		expect(mocks.getIconOrderCaptchaChallenge).toHaveBeenCalledWith(
			USER_ADDRESS,
			SITE_KEY,
			"session-1",
			"prefetched",
...
describe("start: when the provider will not issue a challenge", () => {
	test("surfaces the provider's error to the widget", async () => {
		mocks.getIconOrderCaptchaChallenge.mockResolvedValue(
			challengeResponse({
				error: {
					message: "no session",
	...
	test("keeps no challenge, so a submit afterwards cannot pretend one exists", async () => {
		...
		await expect(harness.manager.start()).resolves.toBeUndefined();
		await expect(
			harness.manager.submitSolution(clicks(), []),
		).rejects.toThrow();
	});
	...
	test("retries against a fresh provider when the challenge call throws", async () => {
		...
		await harness.manager.start();
		expect(mocks.getIconOrderCaptchaChallenge).toHaveBeenCalledTimes(2);
	...
	test("gives up once the retry budget is spent", async () => {
		mocks.getIconOrderCaptchaChallenge.mockRejectedValue(new Error("down"));
		const harness = build({
			frictionlessState: frictionless({ provider: undefined }),
		});
		await harness.manager.start();
		expect(mocks.getIconOrderCaptchaChallenge.mock.calls.length).toBeLessThan(
			5,
		);
		expect(harness.events.onReset).toHaveBeenCalled();
	});
...
describe("submitSolution: refusing to submit without a challenge", () => {
	test("throws when start was never called", async () => {
		const harness = build();
		await expect(
			harness.manager.submitSolution(clicks(), iconOrderEvents()),
		).rejects.toThrow();
	});
	...
	test("clears loading and rethrows when the provider call fails", async () => {
		mocks.submitIconOrderCaptchaSolution.mockRejectedValue(new Error("boom"));
		const harness = build();
		await harness.manager.start();
		await expect(
			harness.manager.submitSolution(clicks(), iconOrderEvents()),
...
describe("submitSolution: what it sends", () => {
	test("sends the ordered clicks and the event trail as given", async () => {
		const harness = build();
		const events = iconOrderEvents();
		await harness.manager.start();
		await harness.manager.submitSolution(clicks(), events);
		const args = submitArgs();
		expect(args[3]).toEqual(clicks());
		expect(args[4]).toEqual(events);
	});

	test("an empty event trail is submitted rather than rejected", async () => {
		const harness = build();
		await harness.manager.start();
		await harness.manager.submitSolution(clicks(), []);
		expect(submitArgs()[4]).toEqual([]);
	});
	...
	test("signs the challenge timestamp with the user's key", async () => {
		...
		expect(submitArgs()[5]).toBe("0xuser-signature");
	});

	test("embeds the checkbox coordinates in the salt", async () => {
		const harness = build();
		await solve(harness);
		const salt = submitArgs()[7];
...
describe("submitSolution: behavioural data", () => {
	...
	test("encrypts and attaches what the collectors gathered", async () => {
		...
		await solve(harness);
		expect(submitArgs()[6]).toBe("encrypted");
	...
	test("submits without behavioural data when encryption fails", async () => {
		...
		expect(submitArgs()[6]).toBeUndefined();
	});
