	...
	const submit = async () => {
		await providerRetry(
			async () => {
				...
				console.log("[DEBUG] About to check frictionlessState (Image)");
				...
				if (
					...
					console.log(
						"[DEBUG] Inside behavioral data collection block (Image)",
					);
					try {
					...
					console.log(
						"[DEBUG] Skipping behavioral data collection - conditions not met (Image)",
					);
				}

				console.log("[DEBUG] About to submit Image captcha solution");
