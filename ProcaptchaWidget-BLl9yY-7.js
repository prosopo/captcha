import { u as u8aToHex$1, z, r as reactExports, l as lightTheme, d as darkTheme, j as jsx, a as jsxs, b as at, e as encodeProcaptchaOutput, A as ApiParams, P as ProcaptchaConfigSchema, R as React, C as ContainerDiv, W as WidthBasedStylesDiv, c as WIDGET_DIMENSIONS, f as WIDGET_PADDING, g as WIDGET_BORDER, h as WIDGET_BORDER_RADIUS, i as WIDGET_INNER_HEIGHT, L as LoadingSpinner, k as WIDGET_URL, m as WIDGET_URL_TEXT, n as Logo } from "./index-DopAH00g.js";
import { s as stringToU8a, _ as _arrayWithHoles, a as _unsupportedIterableToArray, b as _nonIterableRest, I as I18nContext, R as ReportNamespaces, g as getDefaults, c as _defineProperty, d as getI18n, i as instance, P as ProsopoDatasetError, o as objectSpread, E as Extension, e as ProsopoError, h as hexHashArray, f as ProsopoEnvError, j as hexToString, k as getDefaultEvents, t as trimProviderUrl, l as loadBalancer, m as buildUpdateState, n as cryptoWaitReady, p as sleep, r as randomAsHex, q as ProviderApi, u as ExtensionWeb2, v as useProcaptcha, C as Checkbox } from "./index-DSTp4vwh.js";
function stringToHex(value) {
  return u8aToHex$1(stringToU8a(value));
}
...
const CaptchaWidget = ({ challenge, solution, onClick, themeColor }) => {
  ...
  return jsx("div", { style: {
    ...
    return jsx("div", { style: imageStyle, children: jsxs("div", { style: {
      ...
      userSelect: "none",
      fill: "currentcolor"
    }, focusable: "false", color: "#fff", "aria-hidden": "true", viewBox: "0 0 24 24", "data-testid": "CheckIcon", "aria-label": "Check icon", children: jsx("path", { d: "M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" }) }) }) })] }) }, index);
  }) });
};
const isArray = (value) => {
...
class ProsopoCaptchaApi {
  ...
  async submitCaptchaSolution(signer, requestHash, solutions, salt, timestamp, timestampSignature) {
    ...
    try {
      result = await this.providerApi.submitCaptchaSolution(solutions, requestHash, this.userAccount, salt, timestamp, timestampSignature, signature);
    } catch (error) {
...
function Manager(configOptional, state, onStateUpdate, callbacks) {
  ...
  const start = async () => {
    ...
    await fallable(async () => {
      ...
      const captchaApi = new ProsopoCaptchaApi(account.account.address, contract, getRandomProviderResponse, providerApi, config.web2, config.account.address || "");
  ...
  const submit = async () => {
    await fallable(async () => {
      ...
      const submission = await captchaApi.submitCaptchaSolution(signer, challenge.requestHash, captchaSolution, salt, challenge.timestamp, challenge.timestampSignature);
  ...
  const exportData = async (events2) => {
    ...
    if (providerUrlFromStorage) {
      ...
      const getRandomProviderResponse = getRandomActiveProvider();
      const providerUrl2 = trimProviderUrl(getRandomProviderResponse.provider.url.toString());
      providerApi = await loadProviderApi(providerUrl2);
    }
    const providerUrl = storage.getProcaptchaStorage().providerUrl || ((_a = state.captchaApi) == null ? void 0 : _a.provider.provider.url.toString());
    if (!providerUrl) {
      return;
    }
    let account = "";
    try {
      account = getAccount2().account.address;
    } catch (e) {
      console.error(e);
    }
...
const ProcaptchaWidget = (props) => {
  ...
  return jsx("div", { children: jsxs("div", { style: { maxWidth: "100%", maxHeight: "100%", overflowX: "auto" }, children: [jsx(ModalComponent, { show: state.showModal, children: state.challenge ? jsx(CaptchaComponent, { challenge: state.challenge, index: state.index, solutions: state.solutions, onSubmit: manager.submit, onCancel: manager.cancel, onClick: manager.select, onNext: manager.nextRound, themeColor: config.theme ?? "light" }) : jsx("div", { children: "No challenge set." }) }), jsxs(ContainerDiv, { children: [jsx(WidthBasedStylesDiv, { children: jsxs("div", { style: WIDGET_DIMENSIONS, "data-cy": "button-human", children: [" ", jsxs("div", { style: {
    ...
    display: state.loading ? "flex" : "none"
  }, children: jsx("div", { style: { display: "inline-flex" }, children: jsx(LoadingSpinner, { themeColor, "aria-label": "Loading spinner" }) }) })] }) }) }), jsx("div", { style: { display: "inline-flex", flexDirection: "column" }, children: jsx("a", { href: WIDGET_URL, target: "_blank", "aria-label": WIDGET_URL_TEXT, children: jsx("div", { style: { flex: 1 }, children: jsx(Logo, { themeColor, "aria-label": "Prosopo logo" }) }) }) })] })] }) }), config.devOnlyWatchEvents && jsx(Collector, { onProcessData: manager.exportData, sendData: state.sendData, account: state.account })] })] }) });
};
export {
