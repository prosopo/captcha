import { r as reactExports, z, a as at, e as encodeProcaptchaOutput, A as ApiParams, P as ProcaptchaConfigSchema, l as lightTheme, d as darkTheme, j as jsx, b as jsxs, R as React, C as ContainerDiv, W as WidthBasedStylesDiv, c as WIDGET_DIMENSIONS, f as WIDGET_PADDING, g as WIDGET_BORDER, h as WIDGET_BORDER_RADIUS, i as WIDGET_INNER_HEIGHT, L as LoadingSpinner, k as WIDGET_URL, m as WIDGET_URL_TEXT, n as Logo } from "./index-DzoqPQuQ.js";
import { _ as _arrayWithHoles, a as _unsupportedIterableToArray, b as _nonIterableRest, I as I18nContext, R as ReportNamespaces, g as getDefaults, c as _defineProperty, d as getI18n, i as instance, h as hexHashArray, P as ProsopoError, e as ProsopoEnvError, f as ProsopoDatasetError, s as stringToHex, j as hexToString, k as getDefaultEvents, t as trimProviderUrl, l as loadBalancer, m as buildUpdateState, n as cryptoWaitReady, o as sleep, r as randomAsHex, p as ProviderApi, E as ExtensionWeb2, q as ExtensionWeb3, u as useProcaptcha, C as Checkbox } from "./utils-DnE81jQA.js";
function warn() {
...
class ProsopoCaptchaApi {
  ...
  async submitCaptchaSolution(signer, requestHash, solutions, salt, timestamp, providerTimestampSignature) {
    ...
    try {
      result = await this.providerApi.submitCaptchaSolution(solutions, requestHash, this.userAccount, salt, timestamp, providerTimestampSignature, userRequestHashSignature);
    } catch (error) {
      throw new ProsopoDatasetError("CAPTCHA.INVALID_CAPTCHA_CHALLENGE", {
        context: { error }
      });
...
function Manager(configOptional, state, onStateUpdate, callbacks) {
  ...
  const submit = async () => {
    await fallable(async () => {
      ...
      const submission = await captchaApi.submitCaptchaSolution(signer, challenge.requestHash, captchaSolution, salt, challenge.timestamp, challenge.signature.provider.timestamp);
...
const ProcaptchaWidget = (props) => {
  ...
  return jsx("div", { children: jsxs("div", { style: { maxWidth: "100%", maxHeight: "100%", overflowX: "auto" }, children: [jsx(ModalComponent, { show: state.showModal, children: state.challenge ? jsx(CaptchaComponent, { challenge: state.challenge, index: state.index, solutions: state.solutions, onSubmit: manager.submit, onCancel: manager.cancel, onClick: manager.select, onNext: manager.nextRound, themeColor: config.theme ?? "light" }) : jsx("div", { children: "No challenge set." }) }), jsxs(ContainerDiv, { children: [jsx(WidthBasedStylesDiv, { children: jsxs("div", { style: WIDGET_DIMENSIONS, "data-cy": "button-human", children: [" ", jsxs("div", { style: {
    ...
    display: state.loading ? "flex" : "none"
  }, children: jsx("div", { style: { display: "inline-flex" }, children: jsx(LoadingSpinner, { themeColor, "aria-label": "Loading spinner" }) }) })] }) }) }), jsx("div", { style: { display: "inline-flex", flexDirection: "column" }, children: jsx("a", { href: WIDGET_URL, target: "_blank", "aria-label": WIDGET_URL_TEXT, rel: "noreferrer", children: jsx("div", { style: { flex: 1 }, children: jsx(Logo, { themeColor, "aria-label": "Prosopo logo" }) }) }) })] })] }) }), config.devOnlyWatchEvents && jsx(Collector, { onProcessData: manager.exportData, sendData: state.sendData, account: state.account })] })] }) });
};
export {
